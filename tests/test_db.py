...
#     user = User(username='blue', email='blue@pokemon.com',
#                   password='bluepass')
#     session.add(user)
#     await session.flush()
...
# @pytest.mark.asyncio
# async def test_insert_pokemon_to_team(session):
#     user = User(username='gary', password='eevee', email='gary@pokemon.com')
#     session.add(user)
#     await session.flush()
...
#       .where(User.username == 'ash'))
#     db_trainer = db_user.trainer
#     db_pokemon = db_trainer.pokemon[0]

#     assert db_user.email == 'ash@pikachu.com'
